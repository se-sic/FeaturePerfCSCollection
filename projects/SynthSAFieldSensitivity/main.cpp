...
void parseConfigOptions(int argc, char *argv[]) {
  Config.compress = fp_util::isFeatureEnabled(argc, argv, "-c");
  Config.decompress = fp_util::isFeatureEnabled(argc, argv, "-d");
  Config.fastmode = fp_util::isFeatureEnabled(argc, argv, "-1");
  Config.smallmode = fp_util::isFeatureEnabled(argc, argv, "-2");
}
...
int main(int argc, char **argv) {
  ...
  const size_t BUFSIZE = getBufsize(Config.smallmode, Config.decompress);

  if (Config.compress) {

    std::string buff;

    size_t searchlen = getSearchlen(Config.fastmode);
    size_t blocksize = getBlocksize(Config.smallmode);
